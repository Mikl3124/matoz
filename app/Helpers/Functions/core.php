 ...
 * @return string|null
 */
function uploadPostPicture($tmpUploadDir, $file): ?string
{
 ...
 * @return string|null
 */
function uploadPostCustomFieldFile($tmpUploadDir, $file): ?string
{
	if (!$file instanceof \Illuminate\Http\UploadedFile) {
		return null;
	}
	...
	try {
		...
		$disk->put($filePath, \Illuminate\Support\Facades\File::get($file->getrealpath()));
...
function replaceGlobalPatterns(?string $string, bool $removeUnmatchedPatterns = true)
{
	...
	if (Str::contains($string, '{count.listings}')) {
		try {
